...
//            break;
        case Weibo_Netease:
            ...
            [parameters setObject:@"xxxxxxxxx" forKey:@"status"];
            NSString * sting = [[NSBundle mainBundle]pathForResource:@"icon" ofType:@"png"];
            [fileParameters setObject:sting forKey:@"pic"];
            break;
//        case Weibo_Sohu:
...
- (void)publishMessageWithContent:(NSString *)content andBlogId:(enum WeiboId)weiboId andData:(NSData *)data{
    currentRequestId = PublishMessage;
    currentWeiboId = weiboId;
    self.stringSendingContent = content;
    ...
    switch (weiboId) {
        case Weibo_Netease:
            url = @"http://api.t.163.com/statuses/upload.json";
            [parameters setObject:content forKey:@"status"];
            [fileParameters setObject:data forKey:@"pic"];
            break;
        default:
            break;
    }
