...
- (void)cancelAllRequest {
	for (int i = 0; i < [_httpRequests count]; i++) {
		NSDictionary *dicItem = [_httpRequests objectAtIndex:i];
...
- (NSString *)urlSchemeString{
    return [NSString stringWithFormat:@"%@%@", TCURLSchemePrefix, _appKey];
}
...
- (void) managerRequestWithRequest:(TCWBRequest *)request 
	...
	[_httpRequests removeObjectsInArray:tmpArray];
	[tmpArray release];
	...
	[_httpRequests addObject:dicItem];
	[dicItem release];
}
...
- (void)initRequestWithMethodName:(NSString *)methodName
    ...
    NSString *code = nil;
    _temp_delegate = requestDelegate;
    _onSuccessCallback = successCallback;
    _onFailureCallback = failureCallback;

    if ((_refreshToken != nil) || (_expireTime == 0) || _isSSOAuth == YES ) {
//      如果从未登录过或登出过
        if (G_LOGOUT) {
            [self readAuthorizeDataFromKeychain];
        }
        ...
        if ([self isAuthorizeExpired]) {  
            ...
            if (code != nil) {
                ...
                if (range.location != NSNotFound) {
                    ...
                    if([_temp_delegate respondsToSelector:_onFailureCallback])
                        [_temp_delegate performSelector:_onFailureCallback withObject:error];
...
//              如果code返回值为空  
        ...
        if (self.isRefreshTokenSuccess) {
            ...
            if ([_ip_iphone length] == 0) {
                self.ip_iphone = @"10.0.0.172";
            }
...
- (void)logInWithDelegate:(id) requestDelegate  
    ...
    {
        bRet = [WBApi loginApp:_appKey secret:_appSecret reserver:nil];
    }
    ...
    NSDictionary *params = [[NSDictionary alloc]initWithObjectsAndKeys:_appKey,CLIENT_ID,
                            TOKEN, RESPONSE_TYPE,
...
- (void)logInWithAccessToken:(NSString *)theAccessToken 
    ...
    if (![self isAuthorizeExpired]) {
        if ([_temp_delegate respondsToSelector:_onSuccessCallback]) {
            [_temp_delegate performSelector:_onSuccessCallback withObject:nil];
        }
    }
    else{    
        if ([_temp_delegate respondsToSelector:_onFailureCallback]) {
            [_temp_delegate performSelector:_onFailureCallback withObject:nil];
        }
    }
}
...
- (void)request:(TCWBRequest *)req didFailWithError:(NSError *)error{
    ...
    if ([delegate respondsToSelector:_onFailureCallback]) {
        [delegate performSelector:_onFailureCallback withObject:error];
    }
}
...
//            authViewController.requestURLString = urlString;
//            CFRunLoopRef currentRunLoop = CFRunLoopGetCurrent();
//            AuthorizeDelegate *authDelegate = [[AuthorizeDelegate alloc] initWithRunLoop:currentRunLoop delegate:self];
