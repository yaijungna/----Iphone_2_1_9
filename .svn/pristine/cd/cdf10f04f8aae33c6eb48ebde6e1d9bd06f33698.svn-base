...
- (void)networkDataDownloadDataComplete:(FSNetworkData *)sender isError:(BOOL)isError data:(NSData *)data {
	...
	dispatch_async(dispatch_get_global_queue(0, 0), ^(void) {
		...
		dispatch_async(dispatch_get_main_queue(), ^(void) {
            ...
            [[NSNotificationQueue defaultQueue] postNotificationName:(isError ? FSNETWORKDATA_MANAGER_END_DOWNLOADING_ERROR_NOTIFICATION : FSNETWORKDATA_MANAGER_END_DOWNLOADING_COMPLETE_NOTIFICATION) object:self userInfo:userInfo];
			[userInfo release];
		});
...
- (void)networkDataDownloading:(FSNetworkData *)sender maxLength:(long long)maxLength {
	...
	dispatch_async(dispatch_get_global_queue(0, 0), ^(void) {
		...
		dispatch_async(dispatch_get_main_queue(), ^(void) {
			...
			[userInfo setObject:sender.localStoreFilePath forKey:FSNETWORKDATA_MANAGER_LOCALFILEPATH_KEY];
            [[NSNotificationQueue defaultQueue] postNotificationName:FSNETWORKDATA_MANAGER_BEGIN_DOWNLOADING_NOTIFICATION object:self userInfo:userInfo];
			[userInfo release];
		});
	});
	//dispatch_release(queue);
