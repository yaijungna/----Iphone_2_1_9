...
- (NSString *)readDataURLStringFromRemoteHostWithGETDataKind:(GET_DataKind)getDataKind {
	...
	if (getDataKind == GET_DataKind_Refresh) {
        ...
        NSString *strURL = [NSString stringWithFormat:FSONEDAY_NEWS_URL,  self.channelList,self.visitNOList,@"20",[self getMaxRealTimeId]];
        return strURL;
	}
}
-(NSString*)getMaxRealTimeId
{
    long  long min = 1000000000;
    for (FSOneDayNewsObject * obj in  self.objectList) {
        long temp = obj.realtimeid.longLongValue;
        temp < min?min = temp:1;
    }
    return [NSString stringWithFormat:@"%lld",min];
}
...
- (void)initializeSortDescriptions:(NSMutableArray *)descriptions {
	[self addSortDescription:descriptions withSortFieldName:@"realtimeid" withAscending:NO];
}
