...
-(void)setGetWebdata:(FSHTTPGetWebData *)getWebdata
{
    if(getWebdata)
    {
        [getWebdata retain];
    ...
    if (_getWebdata) {
        _getWebdata.parentDelegate = nil;
        [_getWebdata release];
    }
    if (getWebdata) {
        _getWebdata = getWebdata;
    }else
    {
        _getWebdata = nil;
    }
...
-(void)GETdata:(GET_DataKind)httpGetDataKind{
      ...
      if(httpGetDataKind == GET_DataKind_Refresh) {
          ...
          if (self.isGettingList) {
            ...
            self.getWebdata = [FSHTTPGetWebData HTTPGETDataWithURLString:httpGetURLString withDelegate:self];
