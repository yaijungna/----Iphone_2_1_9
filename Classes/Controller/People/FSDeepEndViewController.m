        ...
        if (status == FSBaseDAOCallBack_SuccessfulStatus || status == FSBaseDAOCallBack_BufferSuccessfulStatus) {
            if ([_fs_GZF_DeepEndDAO.objectList count]>0) {
                if (status == FSBaseDAOCallBack_SuccessfulStatus) {
                    ...
                    [tempDict setValue:_deepEndView.deepEndObject.share_img forKey:@"share_img"];
                    [tempDict setValue:_deepEndView.deepEndObject.share_url forKey:@"share_url"];
                    [tempDict setValue:_deepEndView.deepEndObject.share_text forKey:@"share_text"];
